...
interface FlyoutMenuItemProps {
  name: string
  items: Pick<NavigationData, 'items'>['items']
  cta: Pick<NavigationData, 'cta'>['cta']
}
      ...
      >
        <Popover.Panel className="absolute left-1/2 z-10 mt-5 flex w-screen max-w-max -translate-x-1/2 px-4">
          <div className="w-screen max-w-md flex-auto overflow-hidden rounded-3xl bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
            <div className="p-4">
              {items.map(value => {
                const { name, description, href, Icon } = value || {}

                return (
                  <div key={name} className="group relative flex gap-x-6 rounded-lg p-4 hover:bg-gray-50">
